...
| **IA (optionnel)** `translationMode='ai'` | Gemini (BYOK — clé de l'utilisateur) | Meilleure détection + traduction, mais nécessite **sa propre** clé API |
...
par l'utilisateur) offre la qualité max sans coût pour le dev. `translationMode` et
`geminiApiKey` sont dans `chrome.storage.local`.
...
[background.js — service worker MV3]
  ...
  • CAPTURE_FULL_PAGE → scroll + capture + assemblage OffscreenCanvas
      │
      ▼ message { ok, data: { dataURL | images[] } }
      │
...
| `background.js` | Service worker : capture visible + pleine page |
...
| `sidepanel/gemini.js` | Mode IA : `analyzeImage()` (appel Gemini, prompt JSON, erreurs codées) |
...
"content_security_policy": {
  "extension_pages": "script-src 'self' 'wasm-unsafe-eval'; object-src 'self'; connect-src 'self' https://api.mymemory.translated.net https://generativelanguage.googleapis.com"
}
```

- `'wasm-unsafe-eval'` : requis pour Tesseract.js (WASM)
- `connect-src` : MyMemory (mode local) + Gemini (mode IA)
- `storage` : stocke `translationMode` et `geminiApiKey`.

---

## background.js — points importants

- Pattern IIFE `(async()=>{...})(); return true;` sur chaque handler de message
- `captureFullPage(tab)` : scroll + `captureVisibleTab` + assemblage `OffscreenCanvas`
  - scale calculé **avant** la création du canvas (évite dépassement des 16384 px)
  - chunks de 12 000 px max si la page est très longue
  - `dataUrlToBlob()` remplace `fetch(dataUrl)` (interdit dans le SW MV3)
  - `base64FromBlob()` via `arrayBuffer()` + `Uint8Array` + `btoa` (pas de FileReader)
- Réponse toujours `{ ok, data?, error? }`

---

## Pipeline OCR → traduction (le cœur, dans sidepanel.js)

Deux entrées : **Écran visible** (`runVisibleCapture` → sélection → `runAnalysis`)
et **Page entière** (`runFullPagePipeline`, boucle sur les segments). Les deux
finissent par `regionsToBubbles()` puis `renderBubbles()`.
...
- Polarité : si l'image est globalement sombre (moyenne < 128) → inversion
- Export PNG lossless. Aperçu dans le bloc 🔬.
...
### Affichage — `renderBubbles` / `createBlockCard`

Une carte `.block-card` par bulle :
```
🔤 Bulle #1                 conf 87%
🇬🇧  texte OCR        [Copier]
🇫🇷  traduction FR    [Copier]   (⏳… tant que MyMemory n'a pas répondu)
```
`currentBlocks = [{ en, fr, confidence }]` ; `fr` rempli au fil des réponses (utilisé
par « Copier tout »). Garde anti-race via `_ocrGeneration`.
